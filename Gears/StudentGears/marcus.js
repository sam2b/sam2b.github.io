...
function marcusGear(numTeeth, numSpokes) {
...
// Making gear triangles
   ...
   for (i = 0; i < n; i++) {
    ...
    coinEdgeMAR(inCircle,vertices,colors,normals,false);
    coinEdgeMAR(out,vertices,colors,normals,true);
    coinEdgeMAR(rad,vertices,colors,normals,false);
            ...
            if (z > 0)
	         ...
	         if (drawTooth) {
                 ...
                 vertices.push(rad*Math.cos(ang), rad*Math.sin(ang), z,
                               rad*Math.cos(ang+angInc), rad*Math.sin(ang+angInc), z,
                               outRad*Math.cos(ang+angInc), outRad*Math.sin(ang+angInc), z - zDim)
                 ...
                 vertices.push(rad*Math.cos(ang), rad*Math.sin(ang), z,
                               outRad*Math.cos(ang+angInc), outRad*Math.sin(ang+angInc), z -zDim,
                               outRad*Math.cos(ang), outRad*Math.sin(ang), z - zDim);
...
function coinEdgeMAR (x, vertices, colors, normals, isOut) {
    ang = 0;                          // coin edge
    var drawTooth = true;
    var i;
